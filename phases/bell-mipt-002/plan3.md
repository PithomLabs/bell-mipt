# Final Cohesive Implementation Plan: BELL-MIPT-0002A

## Status

Implementation-ready plan.

No physics promotion. No Bell–MIPT bridge claim. No MIPT claim. No measurement claim.
...
Extend the accepted `BELL-MIPT-0001` Go toy with an optional bridge audit that:
...
Use the existing single command:
...
### Pass 1: Preserve BELL-MIPT-0001

Run the accepted `BELL-MIPT-0001` master-equation/equivariance audit exactly as before.
...
```text
Canonical sorted order keeps the compact subsystem/environment basis deterministic.
The requested order is preserved in the report for auditability.
```

---

# 4. Q Index vs Q Config
...
Report and trajectory records may include both if useful.
```
...
Never use a raw bitmask as a matrix index unless the code has explicitly proven `basis[i] == uint64(i)` for that basis.

---

# 5. Time-Step Alignment
...
No jump sampling at RK4 sub-stages in `0002A`.

`sample_every_steps` refers to full time steps, not RK4 sub-stages.
...
# 7. Near-Zero Probability Handling
...
# 9. Site Split and Conditional Projection
...
# 10. Jordan-Wigner / Fermion Sign Rule
...
Also add a permanent warning in bridge report:
...
# 12. Environment-Projected Conditional Vector
...
Fidelity is sufficient for `0002A`. Do not add trace distance in this ticket.

---

# 14. Jump Classification
...
# 15. Primary Conditional Update Ratio
...
# 16. Optional Secondary Metrics

Report these if already tracked, but do not use them for status or debt:
...
# 17. Empirical Trajectory Equivariance
...
norm failure rate <= 50% but too high for reliable metric
...
If norm failure rate > 50%, this may be either `bridge_audit_failed` or `bridge_audit_inconclusive`; prefer `bridge_audit_inconclusive` unless it reflects a coding error.
...
# 19. Environment-Correlated Conditional Update Interpretation
...
The coding agent must implement these tests.
...
Bridge-disabled regression should compare semantic fields, not byte-for-byte JSON.
